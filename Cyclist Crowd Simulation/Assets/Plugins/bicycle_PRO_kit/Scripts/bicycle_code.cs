...
public class bicycle_code : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (!crashed)
        {
            GetComponent<Rigidbody>().linearDamping = GetComponent<Rigidbody>().linearVelocity.magnitude / 210 * airRes; // when 250 bike can easy beat 200km/h // ~55 m/s
            GetComponent<Rigidbody>().angularDamping = 7 + GetComponent<Rigidbody>().linearVelocity.magnitude / 20;
        }

        //determinate the bike speed in km/h
        bikeSpeed = Mathf.Round((GetComponent<Rigidbody>().linearVelocity.magnitude * 3.6f) * 10) * 0.1f; //from m/s to km/h
        ...
        if (!crashed && outsideControls.Vertical > 0.9f && !isReverseOn)// acceleration >0.9f throttle for wheelie	
        {
            ...
            meshRearWheel.GetComponent<Renderer>().material.color = Color.green;
            GetComponent<Rigidbody>().angularDamping = 20;//for wheelie stability
        ...
        {
            GetComponent<Rigidbody>().linearDamping = 0.1f; // when 250 bike can easy beat 200km/h // ~55 m/s
            GetComponent<Rigidbody>().angularDamping = 0.01f;
            crashed = true;

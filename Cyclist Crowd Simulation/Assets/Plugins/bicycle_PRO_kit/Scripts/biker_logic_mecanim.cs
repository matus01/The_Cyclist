...
public class biker_logic_mecanim : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Mathf.Round((bikeRideOn.GetComponent<Rigidbody>().linearVelocity.magnitude * 3.6f) * 10) * 0.1f <= 5 && !bikeStatusCrashed.isReverseOn)
        {//no reverse speed
            reverseSpeed = 0.0f;
            ...
            if (bikeRideOn.transform.localEulerAngles.z <= 15 || bikeRideOn.transform.localEulerAngles.z >= 345)
            {
                ...
                {
                    legOffValue = (5 - (Mathf.Round((bikeRideOn.GetComponent<Rigidbody>().linearVelocity.magnitude * 3.6f) * 10) * 0.1f)) / 5;//need to define right speed to begin put down leg(s)
        ...
        if (Mathf.Round((bikeRideOn.GetComponent<Rigidbody>().linearVelocity.magnitude * 3.6f) * 10) * 0.1f > 5)
        {
            reverseSpeed = 0.0f;
            myAnimator.SetFloat("reverseSpeed", reverseSpeed);
            myAnimator.SetLayerWeight(3, legOffValue);

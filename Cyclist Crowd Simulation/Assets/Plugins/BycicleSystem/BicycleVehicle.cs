...
public class BicycleVehicle : MonoBehaviour
{
	...
	// void setInitialLocRot()
	// {
	// 	rb.position = position;
	// 	rb.rotation = Quaternion.Euler(rotation);
	// }

	public void setInitPositionBicycleVehicle(Vector3 pos, Vector3 rot)
	{
		transform.position = pos;
		transform.rotation = Quaternion.Euler(rot);
		// Debug.Log("setInitPositionBicycle" + pos);
		// rb.position = pos;
	...
	{
		if (rb.linearVelocity.magnitude < 5 ) //We set the limiting factor for the steering thus allowing how much steer we give to the player in relation to the speed
		{			
			maxSteeringAngle = Mathf.LerpAngle(maxSteeringAngle, 50, speedteercontrolTime);
		}
		if (rb.linearVelocity.magnitude > 5 && rb.linearVelocity.magnitude < 10 )
		{			
			maxSteeringAngle = Mathf.LerpAngle(maxSteeringAngle, 30, speedteercontrolTime);
		}
		if (rb.linearVelocity.magnitude > 10 && rb.linearVelocity.magnitude < 15 )
		{			
			maxSteeringAngle = Mathf.LerpAngle(maxSteeringAngle, 15, speedteercontrolTime);
		}
		if (rb.linearVelocity.magnitude > 15 && rb.linearVelocity.magnitude < 20 )
		{			

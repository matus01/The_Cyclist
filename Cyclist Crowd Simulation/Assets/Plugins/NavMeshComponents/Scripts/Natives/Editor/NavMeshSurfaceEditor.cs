...
namespace UnityEditor.AI
{
    ...
    class NavMeshSurfaceEditor : Editor
    {
        ...
        {
            PrefabInstanceStatus prefabInstanceStatus = PrefabUtility.GetPrefabInstanceStatus(navSurface);

            if (prefabInstanceStatus == PrefabInstanceStatus.Connected)
            {
                NavMeshSurface source = PrefabUtility.GetCorrespondingObjectFromSource<NavMeshSurface>(navSurface);
                if (source && source.navMeshData == navSurface.navMeshData)
                {
                    return null;
                }
            }
